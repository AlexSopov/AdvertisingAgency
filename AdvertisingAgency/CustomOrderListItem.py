...
# Класс пользовательского элемента списка
# Формирует все полученные данные в один виджет
class CustomOrderListItem(QWidget):
    ...
    def set_icon_status(self, order_start, order_end):
        ...
        order_end_date = datetime.strptime(order_end, '%Y-%m-%d')

        if current_date.date() < order_start_date.date():
            pix_map = QtGui.QPixmap('images/waiting_icon.png')
            self.label_icon.setToolTip('В ожидании')
        elif current_date.date() > order_end_date.date():
            pix_map = QtGui.QPixmap('images/ready_icon.png')
            self.label_icon.setToolTip('Выполнено')
        else:
            pix_map = QtGui.QPixmap('images/in_process_icon.png')
            self.label_icon.setToolTip('Исполняется')
    ...
    def set_title(self, company_name, billboard_name):
        self.label_title.setText('<b>Заказ на рекламный щит: "{billboard}" организации "{company}"</b>'.
    ...
    def set_date(self, order_date, order_start, order_end):
        ...
        self.label_period.setText('<i>Период размещения рекламы: {from_date} - {till_date}</i>'.
